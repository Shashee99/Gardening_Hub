...
function deletecat(e) {
    ...
    if (confirm(`Are you sure want to delete ${id} ?`)) {
        ...
        ajax.open("POST", "http://localhost/GardeningHub/Productcategories/delete", true);
...
function getData() {
    ...
    ajax.open("POST", "http://localhost/GardeningHub/Productcategories/viewAll", true);
    ajax.send();
...
document.getElementById("addcatbuton").addEventListener('click', () => {
    ...
    if ((catgory != "") && (sucatgory != "")) {
        ...
        ajax.open('POST', 'http://localhost/GardeningHub/Productcategories/insert', false);
        ajax.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        ajax.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
...
document.getElementById("editcatbutton").addEventListener('click' ,()=>{
    ...
    if (newcat.value != "" && newsub.value !=""){
        var ajax = new   XMLHttpRequest();
        ajax.open("POST","http://localhost/GardeningHub/Productcategories/update",true);
        ajax.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
